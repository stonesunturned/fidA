...
class FID(object):
    ...
    def __init__(self, fids, raw_avgs, spectralwidth, txfreq, te, tr, sequence=None, subSpecs=None, rawSubspecs=None,
                 pts_to_left_shift=0, flags=None, dims=None):
        """
    ...
    def set_fid_from_specs(self, newspec):
        ...
        if np.mod(newspec.shape[self.dims['t']], 2) == 0:
            ...
            self.fids = fft(np.roll(fftshift(newspec, axes=self.dims['t']), 1, axis=self.dims['t']),
                            axis=self.dims['t'])
...
if __name__ == '__main__':
    ...
    for debugging
    """
    dirname = '../exampleData/Bruker_press'
    fpath = ''
    outfid, reffid, info_dict = io_loadspec_bruk(dirname, try_raw=True)
    print(outfid.sz)

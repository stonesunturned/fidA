...
def add_phase1(invec, ppm, timeShift, ppm0=4.65, B0=7):
    ...
    """
    f = np.expand_dims((ppm - ppm0) * GAMMAP * B0, axis=tuple(
        range(1, invec.ndim)))  # untested but I think this should work for multidimensional vectors with 1D ppm
...
def op_align_scans(inref, in1, tmax=None, mode='fp'):
    ...
    baseref = np.r_[np.real(inref.fids[np.logical_and(inref.t >= 0, inref.t < tmax)]), np.imag(
        inref.fids[np.logical_and(inref.t >= 0, inref.t < tmax)])]
    if mode == 'f':
        parsFit, pcov = curve_fit(freqShiftComplexNest, in1.fids[np.logical_and(in1.t >= 0, in1.t < tmax)].squeeze(),
                                  baseref.squeeze(), initpars)
        frq = parsFit[0];
        ph = 0;
    elif mode == 'p':
        parsFit, pcov = curve_fit(phaseShiftComplexNest, in1.fids[np.logical_and(in1.t >= 0, in1.t < tmax)].squeeze(),
                                  baseref.squeeze(), initpars)
        ph = parsFit[0];
        frq = 0;
    elif mode == 'fp' or mode == 'pf':
        parsFit, pcov = curve_fit(freqPhaseShiftComplexNest,
                                  in1.fids[np.logical_and(in1.t >= 0, in1.t < tmax)].squeeze(), baseref.squeeze(),
                                  initpars)
        frq = parsFit[0];
        ph = parsFit[1];
    else:
...
def op_alignScans_fd(in1, inref, ppmmin, ppmmax, tmax=0.1, mode='fp'):
    ...
    if not inref.flags['averaged'] or not in1.flags['averaged']:
        raise TypeError(
            'ERROR: Only makes sense to do this after you have combined averages using op_averaging. ABORTING!')
...
def op_freqrange(indat, ppmmin, ppmmax):
    ...
    if np.mod(len(np.nonzero(indvals)), 2) == 0:
        ...
        outdat.fids = fft(np.roll(fftshift(specpart, axes=indat.dims['t']), 1, axis=indat.dims['t']),
                          axis=indat.dims['t'])
    ...
    outdat._ppmmax = ppmmin;
    outdat._ppmmin = ppmmax
...
def op_lorentz(ppm, amp, fwhm, ppm0, base_off=0, ph0=0):
    ...
    y = np.zeros([len(amp), len(ppm)])
    for act, aval in enumerate(amp):
        y[act, :] = np.sqrt(2 / np.pi) * (hwhm[act] - 1j * (ppm - ppm0[act])) / (
                    hwhm[act] ** 2 + (ppm - ppm0[act]) ** 2)
        # Scale it, add baseline, phase by ph0, and take the real part
        y[act, :] = np.real(add_phase(y[act, :] / np.amax(np.abs(y[act, :])) * aval + base_off, ph0))
    y = np.sum(y, axis=0)
    return y.squeeze()
...
def op_multi_peakFit(inspec, ppm, amp, fwhm, ppm0, base_off, ph0, ppmmin=0, ppmmax=4.2, peaktype='lorentz'):
    ...
    parsDict = {'amps': parsFit[:namp], 'fwhms': parsFit[namp:2 * namp], 'ppm0s': parsFit[2 * namp:3 * namp],
                'base_off': parsFit[-2], 'ph0': parsFit[-1]}
    ...
    if indat.flags['averaged']:
        ...
        for m in range(ss):
            ...
            ax1[m].plot(np.r_[:indat.sz[indat.dims['averages']]], zmetric[:, m], '.',
                        np.r_[:indat.sz[indat.dims['averages']]],
                        np.polyval(P[m, :], np.r_[:indat.sz[indat.dims['averages']]]),
                        np.r_[:indat.sz[indat.dims['averages']]],
                        np.polyval(P[m, :], np.r_[:indat.sz[indat.dims['averages']]]).T + nsd, ':')
...
def op_zeropad(indat, zpfact):
    ...
    if indat.flags['zeropadded']:
        ...
        if cflag.lower() == 'y':
            ...
            outdat.fids = np.zeros([zpfact * indat.sz[0], indat.sz[1]]) + 1j * np.zeros(
                [zpfact * indat.sz[0], indat.sz[1]])
